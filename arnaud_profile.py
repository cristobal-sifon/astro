...
def conversion_r500(m500, z, slope = 0.6):
  from mass_conversion_felipe import transform_M500c # Felipe's code for NFW matter profile

  r500 = clusters.rsph(m500, z, ref = '500c', unit = 'Mpc')
  m200 = m500 * transform_M500c(m500, z, (0.27, 0.73, 0.7))
  r200 = clusters.rsph(m200, z, ref = '200c', unit = 'Mpc')
  ...
  print 'z = %.3f' %z
  print 'r500 = %.3f' %r500
  print 'r500/r200 = %.3f (%.3f)' %(r52, 1 / r52)
  print 'm500/m200 = %.3f (%.3f)' %(m52, 1 / m52)
  print '(y500/y200)_sph = %.3f (%.3f)' %(y52s, 1 / y52s)
  print '(y500/y200)_cyl = %.3f (%.3f)' %(y52c, 1 / y52c)
  print ' For y_sph: r500 to r200 --> factor %.3f for a slope %.3f' %(y52s ** slope / m52, slope)
  print ' For y_cyl: r500 to r200 --> factor %.3f for a slope %.3f' %(y52c ** slope / m52, slope)
  print '\ny_{5x500} = %.8f' %y5r500
  print 'y_{500} = %.8f' %y500_sph
  print 'y200_cyl = %.8f' %y200_cyl
  print '(y_{5x500}/y500)_sph = %.3f' %(y5r500 / y500_sph)
  print 'y200_sph/y200_cyl = %.3f (%.3f)' %(y200_sph / y200_cyl, y200_cyl / y200_sph)
  return

def conversion_r200a(m200a, z, slope = 0.6):
  from mass_conversion_felipe import convert_mass # Felipe's code for NFW matter profile

  r200a = clusters.rsph(m200a, z, ref = '200a')
  m200, m500a, m500, m2500 = convert_mass(m200a, z, (0.27, 0.73, 0.7))
  r500  = clusters.rsph(m500, z, ref = '500c')
  r200  = clusters.rsph(m200, z, ref = '200c')
  ...
  print ''
  print 'z = %.3f' %z
  print 'r200a/r200 = %.3f (%.3f)' %(rac, 1 / rac)
  print 'm200a/m200 = %.3f (%.3f)' %(mac, 1 / mac)
  print '(y200a/y200)_sph = %.3f (%.3f)' %(yacs, 1 / yacs)
  print '(y200a/y200)_cyl = %.3f (%.3f)' %(yacc, 1 / yacc)
  print ' For y_sph: r200a to r200 --> factor %.3f for a slope %.3f' %(yacs ** slope / mac, slope)
  print ' For y_cyl: r200a to r200 --> factor %.3f for a slope %.3f' %(yacc ** slope / mac, slope)
  return
...
def profile_sph(M500, r, z, ref = 'absolute', verbose = 'no'):
  ...
  if ref == 'relative':
    x = r
  if verbose == 'yes':
    print 'Calculating Ysph at %.2f*r500 (z=%.3f)' %(x, z)
  Ax = 2.925e-5 * I(x) / h70
  y = Ax * (M500 / 3e14) ** 1.78
  #if z > 0:
...
def profile_cyl(M500, r, z, ref = 'absolute', verbose = 'no'):
  ...
  if ref == 'relative':
    x = r
  if verbose == 'yes':
    print 'Calculating Ycyl at %.2f*r500 (z=%.3f)' %(x, z)
  Bx = 2.925e-5 * J(x) / h70
  y = Bx * (M500 / 3e14) ** 1.78
  #if z > 0:
    #y = y / cosmology.dA(z) ** 2
...
def Yratio(ref1, ref2, proj = 'sph'):
  """
  return the ratio of Y@ref2 to Y@ref1 (i.e., should multiply the value originally given, at ref1)
  """
  m = 1e15; z = 0.5 # unimportant for the ratio
  r1 = clusters.rsph(m, z, ref = ref1)
  r2 = clusters.rsph(m, z, ref = ref2)
  if proj == 'sph':
