...
----------
    Functions take the following parameters:
        ...
        sigma_c : float (optional)
                  critical density, used to correct for the fact that we
                  actually observe the reduced shear, g=gamma/(1-kappa). If
                  not given then this correction is not applied.
...
from numpy import arctan, arctanh, arccos, array, cos, \
                  ...
                  meshgrid, ones, sqrt, sum as npsum, transpose, zeros
...
def esd(x, sigma_s, sigma_c=0):
    ...
    if sigma_c:
        s = sigma(x, sigma_s)
        return (barsigma(x, sigma_s) - s) / (1 - s/sigma_c)
    barsigma(x, sigma_s) - sigma(x, sigma_s)

def esd_trunc5(x, tau, sigma_s, sigma_c=0):
    """
    ...
    """
    if sigma_c:
        s = sigma_trunc5(x, tau, sigma_s)
        return (barsigma_trunc5(x, tau, sigma_s) - s) / (1 - s/sigma_c)
    return barsigma_trunc5(x, tau, sigma_s) - sigma_trunc5(x, tau, sigma_s)

def esd_trunc7(x, tau, sigma_s, sigma_c=0):
    """
    ...
    """
    if sigma_c:
        s = sigma_trunc7(x, tau, sigma_s)
        return (barsigma_trunc7(x, tau, sigma_s) - s) / (1 - s/sigma_c)
    return barsigma_trunc7(x, tau, sigma_s) - sigma_trunc7(x, tau, sigma_s)

def esd_sharp(x, tau, sigma_s, sigma_c=0):
    """
    ...
    """
    if sigma_c:
        s = sigma_sharp(x, tau, sigma_s)
        return (barsigma_sharp(x, tau, sigma_s) - s) / (1 - s/sigma_c)
...
def esd_offset(x, xoff, n, sigma_s, x_range, angles, sigma_c=0,
               interp_kind='slinear'):
    """
...
def sigma_offset(x, xoff, n, sigma_s, x_range, angles,
                 interp_kind='slinear'):
    s = sigma_distribution(xoff, n, sigma_s, x_range, angles, interp_kind)
    return s(x[1:]) / (2*3.14159265)
