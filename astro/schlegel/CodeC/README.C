...
This set of self-contained C routines is for reading the SFD dust maps.
...
for reading Lambert-projection files is lambert_getval.  You may link
directly to dust_getval() in C (as done in dust_getval.c), or link to
fort_lambert_getval() in Fortran (as done in testf_lambert.f).
...
Available maps
--------------
   The following maps are available by setting "map" in the call to
   "dust_getval":
...
Calling sequence
----------------
   dust_getval gall galb map=map infile=infile outfile=outfile \
    interp=interp noloop=noloop verbose=verbose ipath=ipath
...
---------------
   ...
   In this example, the returned value should be E(B-V) = 0.2926 mag.
...
---------------
   ...
   noloop:     Set this flag to "y" to read all values at once by reading
               the entire image into memory at once (or the smallest possible
               subimage.  This is a faster option for reading a large
               number of values, but requires reading up to a 64 MB image
