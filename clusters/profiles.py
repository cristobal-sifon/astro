...
def read_nfw(m, z, dm=0, ref_in='500c', ref_out='200c'):
    """
    read the file nfw_conversion.dat located in this folder, and
    interpolate linearly between the closest values of m for ref_in.
    The mass range in the file is M200a=[1e14, 5e15] and includes the
    redshift range [0, 2] in steps of 0.01. The values in this file
    assume the Duffy et al. concentration relation.
    ...
    data = readfile.dict(path + '/nfw_conversion.dat',
                        cols=numpy.arange(1, 9), include=z0,
                        lower=True)
    m_in = data[col_in]
    m_out = data[col_out]
    # identify closest value
    i = numpy.argmin(numpy.absolute(m - data[col_in]))
    # do linear interpolation
    if m == m_in[i]:
        mass = m_out[i]
    elif m > m_in[i]:
        order = numpy.array([i, i + 1])
        mass = scipy.interp(m, m_in[order], m_out[order])
    else:
        order = numpy.array([i - 1, i])
        mass = scipy.interp(m, m_in[order], m_out[order])

    # do the same for dm
    if dm > 0:
        # identify closest value
        i = numpy.argmin(abs(dm - data[col_in]))
        # do linear interpolation
        if dm == m_in[i]:
            return mass, m_out[i]
        elif dm > m_in[i]:
