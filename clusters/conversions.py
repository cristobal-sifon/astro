...
def Msph(r, z, dr=0., ref='200c', unit='Msun', r_unit='kpc'):
  """
  Calculates the mass of a cluster with radius r, assuming it is spherical.
  ...
  if r_unit == 'kpc':
    r = r / 1000
  m = rho * 4 * scipy.pi / 3 * r ** 3 # [M] = Msun

  if dr != 0.:
    dm = rho * 4 * scipy * r ** 2 # [dM] = Msun
...
def rsph(m, z, dm=0., ref='200c', unit='Mpc'):
  """
  Returns r, the radius that contains a density *contrast* times the critical 
  density, assuming a spherical distribution, given a mass m,
  
  m = (4·pi/3)·r³·(contrast·rho_c)

  If an error on the mass is provided, also returns the error in r.
    *Input:
  m: the mass of the cluster, in solar masses. Can be an uncertainties.ufloat
     as well, in which case *r* will be returned with the same type.
  ...
  #        m = 4*pi/3 * rho*r**3
  # --> r**3 = m / (4*pi/3) / rho
  r = (m / (4 * scipy.pi / 3) / rho) ** (1. / 3.) # [r] = Mpc
  if dm != 0. and type(m) != uncertainties.Variable:
    dr = 1 / (4 * scipy.pi) * 1 / (rho * r ** 2) * dm # [dr] = Mpc
    if unit == 'kpc':
      return 1e3 * r, 1e3 * dr
    if unit == 'Mpc':
      return r, dr
